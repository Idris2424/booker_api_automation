...
class TestAuth:
    ...
    def test_invalid_password_token(self, auth_steps, booking_for_auth_test):
        invalid_token = auth_steps.get_token_with_invalid_credentials(AUTH_DATA_WRONG_PASS)
        auth_steps.verify_invalid_token(invalid_token)

    @allure.title("TC_AUTH_003: Токен с несуществующим пользователем невалиден")
    def test_fake_user_token(self, auth_steps, booking_for_auth_test):
    ...
    def test_valid_token_allows_protected_request(self, auth_steps, booking_for_auth_test):
        auth_steps.put_booking_with_valid_token()

    @allure.title("TC_AUTH_005: Использование неверного токена возвращает 403")
    def test_invalid_token_returns_403(self, auth_steps, booking_for_auth_test):
    ...
    def test_no_token_returns_403(self, auth_steps, booking_for_auth_test):
